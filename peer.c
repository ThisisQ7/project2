...
typedef struct sender_state{
  ...
  struct timeval last_rtt_sent;       // For RTT sampling
  struct timeval last_window_growth;  // To control window growth timing
...
void clear_sender_state(sender_state_t *state) {
    ...
    gettimeofday(&state->last_window_growth, NULL);
    gettimeofday(&state->last_rtt_sent, NULL);
    state->srtt = 0;
    ...
    // Record start time -> might need to comment
...
sender_state_t *get_sender_state(struct sockaddr_in *from) {
  ...
  gettimeofday(&new_state->last_window_growth, NULL);
  gettimeofday(&new_state->last_rtt_sent, NULL);
  new_state->srtt = 0;
...
void start_data_transmission(int id, struct sockaddr_in from, int sock) {
  ...
  send_data_packet(from, state, sock, 1, false);
...
void ack_handler(packet_t *pack, struct sockaddr_in from, int sock) {
  ...
  if (pack_ack == state->last_packet_acked) {
    ...
    struct timeval now;
    gettimeofday(&now, NULL);
    double sample_rtt = (now.tv_sec - state->last_rtt_sent.tv_sec) * 1000.0 +
                        (now.tv_usec - state->last_rtt_sent.tv_usec) / 1000.0; 
    
    if (state->srtt == 0) {
        state->srtt = sample_rtt;
    } else {
        state->srtt = (1 - 0.125) * state->srtt + 0.125 * sample_rtt;
    }

    double elapsed_ms = (now.tv_sec - state->last_window_growth.tv_sec) * 1000.0 +
    (now.tv_usec - state->last_window_growth.tv_usec) / 1000.0;
    ...
    if (state->congestion_state == SLOW_START) {
      ...
      if (elapsed_ms >= state->srtt) {
        state->window_size += 1;
        gettimeofday(&state->last_window_growth, NULL);
        DEBUG_PRINT("RTT-based growth: Increased window to %d after %.2fms\n",
                    state->window_size, elapsed_ms);
        } 
    }
