 ...
 */
void send_data_packet(struct sockaddr_in from, sender_state_t *state, int sock, int seq_num, bool is_retransmit) {
  ...
  if (!is_retransmit && seq_num > state->last_packet_sent){
...
void start_data_transmission(int id, struct sockaddr_in from, int sock) {
  ...
  for (i = 0; i < WINDOW_SIZE; i++) {
    ...
    int seq_num = i + 1;
    packet_t *curr_pack = state->window[seq_num%WINDOW_SIZE];
  ...
  int first_seq_num = 1;
  packet_t *curr_pack = state->window[first_seq_num];

  curr_pack->header.packet_len = htons(HEADERLEN + PAYLOAD_SIZE);
  state->data_offset = (state->chunk_id * BT_CHUNK_SIZE);
  send_data_packet(from, state, sock, first_seq_num, false);
}
...
void ack_handler(packet_t *pack, struct sockaddr_in from, int sock){
  ...
  if (pack_ack <= state->last_packet_acked) {
    ...
    if (state->dup_ack_count >= 3) {
      ...
      int missing_seq = state->last_packet_acked + 1;
      send_data_packet(from, state, sock, missing_seq, true);      
    }
    ...
    // Clear timers for all ACKed packets
    for (int i = state->last_packet_acked + 1; i <= pack_ack; i++) {
      struct timeval *acked_timer = &state->timers[i % WINDOW_SIZE];
      acked_timer->tv_sec = 0;
      acked_timer->tv_usec = 0;
    }
    ...
    for (seq_num = state->last_packet_sent + 1; seq_num <= state->last_packet_available; seq_num++){
      ...
      int data_offset = (state->chunk_id * CHUNK_SIZE) + ((seq_num - 1) * PAYLOAD_SIZE);
      if (data_offset >= ((state->chunk_id + 1 ) * state->chunk_size)){
        DEBUG_PRINT("Stopping send at seq %d: would go beyond chunk end\n", seq_num);
        break;
      }      
      ...
      send_data_packet(from, state, sock, seq_num, false);
    }
  }
}
...
void check_timeout(){
  if (sender_states != NULL){
    ...
    while(state != NULL){
      packet_t *last_acked_pkt = state->window[(state->last_packet_acked + 1) % WINDOW_SIZE];
      int packet_len = ntohs(last_acked_pkt->header.packet_len);
      int header_len = ntohs(last_acked_pkt->header.header_len);
      int payload_size = packet_len - header_len;
      size_t next_ack_offset = (state->last_packet_acked * payload_size);

      if (next_ack_offset >= state->chunk_size) {
        DEBUG_PRINT("Transmission complete for peer %s:%d — skipping timeout check\n",
                    inet_ntoa(state->addr.sin_addr), ntohs(state->addr.sin_port));
        state = state->next;
        continue;
      }
      ...
      for (i = state->last_packet_acked + 1; i <= state->last_packet_sent; i++){
        ...
        struct timeval *packet_time = &state->timers[idx];
        packet_t *packet = state->window[idx];

        int packet_len = ntohl(packet->header.packet_len);
        int header_len = ntohl(packet->header.header_len);
        int payload_size = packet_len - header_len;

        if (packet_time->tv_sec == 0 && packet_time->tv_usec == 0){
          continue;
        }
        ...
        if (time_diff > PACKET_TIMEOUT){
          ...
          int data_offset = (state->chunk_id * CHUNK_SIZE) + ((i - 1) * (payload_size));
          if (data_offset >= (state->chunk_id + 1) * CHUNK_SIZE) {
            DEBUG_PRINT("Skipping retransmit: seq %d would overflow chunk boundary\n", i);
            continue;
          }

          state->window[i % WINDOW_SIZE]->header.seq_num = htonl(i);
          state->data_offset = data_offset;
          
          send_data_packet(state->addr, state, sock, i , true);
