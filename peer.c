...
void clear_sender_state(sender_state_t *state){
  ...
  int i;
  for (i = 0; i < WINDOW_SIZE ; i++) {
    printf("Freeing up window index i: %d\n", i);
    free(state->window[i]);
  }
}
...
chunk_t *find_req_chunk(uint8_t *hash){
  ...
  for (i = 0; i < requested_num; i++){
    ...
    printf("Checking anything else about requested chunk, id:%d\n", requested_chunks[i].id);
    char hashout1[40];
    char hashout2[40];
    ...
    if (strcmp(hashout1, hashout2) == 0) {
      printf("Match found for chunk %d\n", i);
            
      // ADD THIS DEBUGGING STATEMENT
      printf("Returning pointer: %p (requested_chunks[%d])\n", (void*)&requested_chunks[i], i);
